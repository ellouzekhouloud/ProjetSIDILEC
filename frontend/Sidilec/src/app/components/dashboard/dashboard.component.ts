...
modeFiltre: string = '';
 ...
 applyFilter() {
  if (!this.modeFiltre) {
    //alert('Veuillez sélectionner un mode de filtre.');
    return;
  }
  ...
  if (this.modeFiltre === 'custom') {
    ...
    this.dashboardService.getQuantitesParFournisseur(this.modeFiltre, undefined, this.startDate, this.endDate)
      .subscribe({
        next: (data) => {
  ...
  if (!this.typePeriode || !this.dateFiltre) {
    //alert('Veuillez sélectionner une date et un type de période.');
    return;
  }
