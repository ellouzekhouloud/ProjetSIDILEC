...
@Input() modeFiltre: string = '';  // 'custom' ou 'standard'
  @Output() modeFiltreChange = new EventEmitter<string>();
  ngAfterViewInit() {
    this.createChart();
  }
  ...
  // Quand on modifie la période personnalisée
   onModeChange() {
    if (this.modeFiltre === 'custom') {
      // On vide le filtre standard quand on choisit custom
      this.typePeriode = '';
      this.dateFiltre = '';
      this.typePeriodeChange.emit(this.typePeriode);
      this.dateFiltreChange.emit(this.dateFiltre);
    } else if (this.modeFiltre === 'standard') {
      // On vide la période personnalisée quand on choisit standard
      this.startDate = '';
      this.endDate = '';
      this.startDateChange.emit(this.startDate);
      this.endDateChange.emit(this.endDate);
    }
    this.modeFiltreChange.emit(this.modeFiltre);
    this.filterChanged.emit();
  }

  onCustomDateChange() {
    if (this.modeFiltre !== 'custom') return; // ignore si pas en mode custom
  ...
  onStandardFilterChange() {
    if (this.modeFiltre !== 'standard') return; // ignore si pas en mode standard
